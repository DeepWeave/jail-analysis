...
function createCaseFromHtml(tr) {
  ...
  for (let i = 0; i < tr.children.length; i++) {
    ...
    if (i === 7) { // All-county search, adds 1 column
...
function computeSearchUrl(state, start, navIndex, submit) {
  ...
  return `https://www1.aoc.state.nc.us/www/calendars.Criminal.do?county=999&court=BTH+&${queryParams}&start=${start}&navindex=${navIndex}&fromcrimquery=yes&submit=${submit}`;
}
  ...
  for (item in defendants) {
    ...
    let d = {
      ...
      cases: caselist.map(c => {
        ...
        try {
          if (c.linkToCaseDetails) {
            console.log(detailsUrl.toString())
            const axiosInstance2 = axios.create();
            axiosCookieJarSupport(axiosInstance2);
            axiosInstance2defaults.jar = new tough.CookieJar(); // Make sure 
            let resp = await axiosInstance2.get(detailsUrl.toString(), {withCredentials: true});
            console.log('DETAILS')
